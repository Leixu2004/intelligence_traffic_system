...
| # | 段 | 状态 | 耗时(ms) | 说明 |
| --- | --- | --- | --- | --- |
| 1 | flink_window | ok | 37 | level=PURPLE source=timescaledb |
| 2 | forecast | degraded | 0 | prediction_failed:PredictionInputMissing |
| 3 | vllm_analysis | degraded | 0 | origin=rule_fallback |
| 4 | agent_plan | degraded | 8463 | prediction_failed:PredictionInputMissing; advise_failed:VllmUnavailableError，回落规则模板; recommendation=rule_fallback（模板文案，非模型生成） |
| 5 | screen_push | ok | 1 | path=D:\CODE\TRAFFIC\intelligent_transportation-main\intelligent_transportation-main\intel-transportation\data\integration\screen_push.jsonl |

## 数据流转

1. Flink 窗口：相机 `CAM-02` / 卡口 `CAM-02` @ `2026-09-21T14:25:00`，均速 26.9 km/h，车辆数 7，级别 `PURPLE`（来源 `timescaledb`）
2. 预测：本次未产出（见下降级明细）
